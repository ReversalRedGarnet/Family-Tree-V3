...
export default function RelationshipModal({
  ...
  const siblingSuggestion = useMemo(
    () => inferSiblingType(personA, personB, relationships),
    [personA, personB, relationships]
  );
  // The effect below keys off the primitive, not the object: a recompute
  ...
  useEffect(() => {
    ...
    setStartDate('');
    setEndDate('');
    // Start on whatever the parents already on the board imply, when they
    // imply anything at all. Still fully overridable below.
            ...
            {siblingSuggestion && (
              <p className="mt-1.5 text-xs leading-snug text-mist">
                {siblingType === siblingSuggestion.type
                  ...
                  : `The board suggests ${
                      SIBLING_TYPES.find((t) => t.id === siblingSuggestion.type)?.label.toLowerCase() ||
                      siblingSuggestion.type
